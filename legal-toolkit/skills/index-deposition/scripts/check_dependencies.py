#!/usr/bin/env python3
"""
Check and auto-install dependencies for the legal-depo-indexer skill.

Uses importlib.util.find_spec() to probe packages WITHOUT importing them,
so heavy libraries (faster-whisper, PyTorch) are never loaded during the check.
...
def main():
    ...
    # System deps (ffmpeg — required for video, needed for many audio formats)
    ...
    if missing_sys:
        ...
        for name, hint in missing_sys:
            if sys.platform == "darwin" and shutil.which("brew"):
                ...
                print("(Required for video files and most audio formats.)")
                continue
            if result.returncode != 0:
    ...
    if missing_optional:
        ...
        print("  Deposition indexing works fine without it — just no speaker labels.")

    # HuggingFace token info (needed for pyannote diarization models)
    ...
    if not hf_token:
        ...
        print("  Indexing works fine without it — just no speaker labels.")
