#!/usr/bin/env python3
"""Check and auto-install dependencies for the legal-ediscovery skill."""
...
# Optional deps — hard to install on some platforms
OPTIONAL_DEPS = {
    "pypff": "libpff-python",
}
...
def main():
    ...
    if missing_optional:
        print(f"Optional: attempting to install {', '.join(missing_optional)}")
        try:
            if install_python_packages(missing_optional):
                installed_something = True
        except Exception:
            print(f"Optional: {', '.join(missing_optional)} unavailable (this is OK)")
            print("  libpff-python enables .pst file support but is not required.")
